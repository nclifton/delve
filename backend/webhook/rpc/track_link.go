...
func (s *Webhook) PublishLinkHit(p types.PublishLinkHitParams, r *types.NoReply) error {
	webhooks, err := s.db.FindByEvent(p.AccountID, "link_hit")
	if err != nil {
		return err
	}
	...
	for _, w := range webhooks {
		err = s.db.Publish(msg.WebhookMessageSpec{
			...
			Payload:   msg.WebhookBody{Event: "link_hit", Data: p},
