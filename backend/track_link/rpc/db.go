...
func (db *db) InsertTrackLink(ctx context.Context, accountID, messageID, messageType, url string) (*types.TrackLink, error) {
	tracklink := types.TrackLink{}
...
func (db *db) FindTrackLinkByTrackLinkID(ctx context.Context, tracklinkID string) (*types.TrackLink, error) {
	tracklink := types.TrackLink{}
	err := db.postgres.QueryRow(
		ctx,
		`SELECT id, account_id, message_id, message_type, track_link_id, created_at, updated_at, url, hits
		FROM track_link
		WHERE track_link_id = $1`,
		tracklinkID,
	).Scan(
		&tracklink.ID,
		&tracklink.AccountID,
...
func (db *db) IncrementTrackLinkHits(ctx context.Context, tracklinkID string) (*types.TrackLink, error) {
	tracklink := types.TrackLink{}
	err := db.postgres.QueryRow(
		ctx,
		`UPDATE track_link
